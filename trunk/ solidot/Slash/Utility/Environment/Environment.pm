...
#		{ return_only => [qw( ipid subnetid classbid ip )] });
	...
	# hacked for Chinese data format --Jesse 2006/05/27
	# also added 2 vars in admin panel "aton_on" "aton_around"
	if ($user->{'format'} eq '%l ish') {    # %i
        $user->{aton} = $constants->{'aton_around'}; # Slash::getData('atonish');
    } else {
        $user->{aton} = $constants->{'aton_on'}; # Slash::getData('aton');
    }
...

{
	...
	my %special = (
		...
		query		=> sub { $_[0] =~ s|[\000-\040<>]+| |g;  # hacked for chinese search. Jesse 2006/05/19
			        	 $_[0] =~ s|\s+| |g;				},
